...
function buildAddressCreateData(
  ...
  if (data.portCode !== undefined) {
    addressData.portCode = data.portCode;
  }

  if (data.terminalCode !== undefined) {
    addressData.terminalCode = data.terminalCode;
  }

  return addressData;
}

function buildAddressUpdateData(
  data: SeedAddressInput,
...
async function seedAddress(data: SeedAddressInput) {
  ...
  if (existingAddress) {
    return prisma.address.update({
      ...
      data: buildAddressUpdateData(data),
    });
  }

  return prisma.address.create({
    data: buildAddressCreateData(data),
  });
}
...
async function seedSettings(): Promise<void> {
  const settings: SeedSettingInput[] = [
    {
...
async function seedVepco(): Promise<void> {
  const vepco = await prisma.customer.upsert({
    ...
    update: {
      ...
      notes:
        "Seed customer. Uses fixed upper-bound tariff table and one-way billable kilometers.",
    },
    create: {
      name: "Vepco Transport B.V.",
      status: "ACTIVE",
...
async function seedMsi(): Promise<void> {
  const msi = await prisma.customer.upsert({
    ...
    update: {
      ...
      notes:
        "Seed customer. Uses price per kilometer on total route kilometers.",
    },
    create: {
      name: "MSI Transport B.V.",
      status: "ACTIVE",
...
async function seedManualCustomers(): Promise<void> {
  const manualCustomerNames = ["Eucon", "Maersk", "MSC"];
  ...
  for (const customerName of manualCustomerNames) {
    const manualCustomer = await prisma.customer.upsert({
      ...
      update: {
        ...
        notes:
          "Seed customer. Manual pricing until customer-specific tariff is configured.",
      },
      create: {
        name: customerName,
        status: "ACTIVE",
  ...
  console.log("Seeded manual customers: Eucon, Maersk, MSC");
}

async function seedAddresses(): Promise<void> {
  const addresses: SeedAddressInput[] = [
    {
      name: "ECT Delta",
  ...
  for (const address of addresses) {
    await seedAddress(address);
  }

  console.log(
    `Seeded addresses: ${addresses.map((address) => address.name).join(", ")}`,
  );
}
